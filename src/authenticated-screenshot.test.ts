      ...
      test(testName, async ({ page }) => {
        ...
        try {
          ...
          if (fs.existsSync("github-auth-state.json")) {
            ...
            console.log(
              `⚠️  No authentication state found. Run 'npm run auth' first.`
            );
          ...
          console.log(
            `🔧 Preparing authenticated page for ${config.width}x${config.height}`
          );
          await preparePage(page, finalConfig);

          // Navigate to URL with robust error handling
          console.log(
            `📸 Capturing authenticated ${url} at ${config.width}x${config.height}`
          );
          ...
          const pageInfo = await page.evaluate(() => {
            ...
            const isAuthenticated =
              authIndicators.notOnLogin && authIndicators.urlOk;
          ...
          if (!pageInfo.isAuthenticated && url.includes("/settings")) {
            console.log(
              `⚠️  Expected authentication for ${url} but got redirected to ${pageInfo.url}`
            );
          }
